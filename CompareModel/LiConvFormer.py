...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.drop = 0.2
        self.dim = 64
        self.LEFL_num_layers = 2

        self.in_layer = nn.Sequential(
            nn.AvgPool1d(2, 2),
            ConvBNReLU(self.input_dim, self.dim, kernel_size=15, stride=2)
        )
        self.LFEL_list = nn.ModuleList()
        for i in range(self.LEFL_num_layers):
            self.LFEL_list.append(
                LFEL(self.dim * (2 ** i),self.dim * 2 ** (i + 1), self.drop)
            )
        
        self.gap = nn.AdaptiveAvgPool1d(1)

        self.out_layer = nn.Linear(2 ** (self.LEFL_num_layers) * self.dim, self.num_class)

    def forward(self, x):
        # x: B, T, N
        x = x.transpose(1, 2)
        x = self.in_layer(x)
        for _, LEFL_layer in enumerate(self.LFEL_list):
            x = LEFL_layer(x) # B, C, T
        
        x_pool = self.gap(x)
        logits = self.out_layer(x_pool.view(x_pool.shape[0], -1))
        return logits
'''
doi: https://doi.org/10.1016/j.eswa.2023.121338
Paper name: LiConvFormer: A lightweight fault diagnosis framework using separable multiscale convolution and broadcast self-attention
'''
